        ...
        try:
            ...
            for article in all_articles:
                data.append({
                    ...
                    'category': article.category.title,
                    'auther': article.author.user.first_name + ' ' + article.author.user.last_name,
                    'promote': article.promote, 
                })

            return Response(data=data, status=status.HTTP_200_OK)
        except:
            return Response({'starus':'Internal Server Error'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
